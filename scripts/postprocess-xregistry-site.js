...
<configuration>
    <system.webServer>
        <httpRedirect enabled="false" />
            ...
            const detailsEntry = parentEntries.find(e => e.isFile() && /\$details\.json$/i.test(e.name));
            if (detailsEntry) {
...
<configuration>
    <system.webServer>
        <httpRedirect enabled="true"
            ...
            const isVersionDirectory = path.basename(dir) === 'versions' && /^\d+\.\d+\.\d+$/.test(entry.name);
...
<configuration>
    <system.webServer>
        <httpRedirect enabled="true"
                ...
                await fs.writeFile(path.join(subdir, 'web.config'), defaultConfig, 'utf8');
            }
